/**
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 *
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package org.openapitools.client.models


import com.squareup.moshi.Json
import com.squareup.moshi.JsonClass

/**
 * 
 *
 * @param type 
 * @param contactId 
 * @param about 
 * @param avatarUrl 
 * @param `data` 
 * @param dataLabel 
 * @param data1 
 * @param data1Label 
 * @param data2 
 * @param data2Label 
 * @param data3 
 * @param data3Label 
 * @param data4 
 * @param data4Label 
 * @param data5 
 * @param data5Label 
 * @param data6 
 * @param data6Label 
 * @param data7 
 * @param data7Label 
 * @param data8 
 * @param data8Label 
 * @param data9 
 * @param data9Label 
 * @param availableForHire 
 * @param careerLevel 
 * @param experienceInYears 
 * @param currentSalary 
 * @param minSalaryExpectation 
 * @param maxSalaryExpectation 
 * @param currencyId 
 */


data class JobApplicantProfileUpdateDto (

    @Json(name = "type")
    val type: kotlin.String? = null,

    @Json(name = "contactId")
    val contactId: kotlin.String? = null,

    @Json(name = "about")
    val about: kotlin.String? = null,

    @Json(name = "avatarUrl")
    val avatarUrl: kotlin.String? = null,

    @Json(name = "data")
    val `data`: kotlin.String? = null,

    @Json(name = "dataLabel")
    val dataLabel: kotlin.String? = null,

    @Json(name = "data1")
    val data1: kotlin.String? = null,

    @Json(name = "data1Label")
    val data1Label: kotlin.String? = null,

    @Json(name = "data2")
    val data2: kotlin.String? = null,

    @Json(name = "data2Label")
    val data2Label: kotlin.String? = null,

    @Json(name = "data3")
    val data3: kotlin.String? = null,

    @Json(name = "data3Label")
    val data3Label: kotlin.String? = null,

    @Json(name = "data4")
    val data4: kotlin.String? = null,

    @Json(name = "data4Label")
    val data4Label: kotlin.String? = null,

    @Json(name = "data5")
    val data5: kotlin.String? = null,

    @Json(name = "data5Label")
    val data5Label: kotlin.String? = null,

    @Json(name = "data6")
    val data6: kotlin.String? = null,

    @Json(name = "data6Label")
    val data6Label: kotlin.String? = null,

    @Json(name = "data7")
    val data7: kotlin.String? = null,

    @Json(name = "data7Label")
    val data7Label: kotlin.String? = null,

    @Json(name = "data8")
    val data8: kotlin.String? = null,

    @Json(name = "data8Label")
    val data8Label: kotlin.String? = null,

    @Json(name = "data9")
    val data9: kotlin.String? = null,

    @Json(name = "data9Label")
    val data9Label: kotlin.String? = null,

    @Json(name = "availableForHire")
    val availableForHire: kotlin.Boolean? = null,

    @Json(name = "careerLevel")
    val careerLevel: JobApplicantProfileUpdateDto.CareerLevel? = null,

    @Json(name = "experienceInYears")
    val experienceInYears: kotlin.Int? = null,

    @Json(name = "currentSalary")
    val currentSalary: kotlin.Double? = null,

    @Json(name = "minSalaryExpectation")
    val minSalaryExpectation: kotlin.Double? = null,

    @Json(name = "maxSalaryExpectation")
    val maxSalaryExpectation: kotlin.Double? = null,

    @Json(name = "currencyId")
    val currencyId: kotlin.String? = null

) {

    /**
     * 
     *
     * Values: Junior,Intermediate,MidLevel,Executive,Senior
     */
    @JsonClass(generateAdapter = false)
    enum class CareerLevel(val value: kotlin.String) {
        @Json(name = "Junior") Junior("Junior"),
        @Json(name = "Intermediate") Intermediate("Intermediate"),
        @Json(name = "MidLevel") MidLevel("MidLevel"),
        @Json(name = "Executive") Executive("Executive"),
        @Json(name = "Senior") Senior("Senior");
    }

}

