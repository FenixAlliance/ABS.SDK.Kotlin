...
> EmptyEnvelope patchPaymentTermAsync(paymentTermId, tenantId, apiVersion, xApiVersion, operation)
...
try {
    val result : EmptyEnvelope = apiInstance.patchPaymentTermAsync(paymentTermId, tenantId, apiVersion, xApiVersion, operation)
    println(result)
} catch (e: ClientException) {
